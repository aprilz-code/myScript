...
def main(_user, _passwd, _step):
    ...
    if _step == '':
        ...
        _step = str(random.randint(18000, 19999))
...
class ToPush:
   ...
   #     self.pkey = _pkey
    ...
    def to_push_wx(pkey,push_msg):
        """
        推送server酱接口
        """
        push_wx(pkey, push_msg)

    def to_push_server(pkey,push_msg):
        """
        推送消息到微信接口
        """
        push_server(pkey, push_msg)

    def to_push_tg(pkey, push_msg):
        """
        推送消息到TG接口
        """
        try:
            token, chat_id = pkey.split('@')
            push_tg(token, chat_id, push_msg)
        except ValueError:
            print('tg推送参数有误！')

    # def to_wxpush(pkey, push_msg):
    #     """
    #     企业微信推送接口
    #     """
    #     try:
    #         usr, corpid, corpsecret, *agentid = pkey.split('-')
    #         if agentid:
    ...
    def to_push_pushplus(pkey, push_msg):
        """
        接口
        """
        push_pushplus(pkey, push_msg)
...
if __name__ == "__main__":
    ...
    try:
        ...
        # 登录密码
        passwd = os.getenv('MI_PWD')
        # 要修改的步数，直接输入想要修改的步数值，0为随机步数
        step = os.getenv('MI_STEP')
    ...
    setp_list = step.split('&')
    pkey_list = pkey.split('&')


    if len(user_list) == len(passwd_list) and len(user_list) == len(setp_list) and len(user_list) == len(pkey_list):

        for user, passwd, setp,pkey in zip(user_list, passwd_list,setp_list,pkey_list):
            push_msg = ''
            setp_array = setp.split('-')
            if len(setp_array) == 2:
